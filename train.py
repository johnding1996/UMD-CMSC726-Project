...
model = DiscreteFlow(flow_num_scf_layers=2, flow_num_hidden_layers=2, flow_hidden_dim=16,
...
def flow_loss_function(epsilon, logdet) :
    ...
    return ratio*log_p_z, ratio*log_p_eps, ratio*logdet

def train(epoch):
    model.train()
    train_loss, train_logdet, train_log_p = 0, 0, 0
    ...
    for batch_idx, data in enumerate(train_iter):
        ...
        if epoch <= 20:
            ...
            loss = vae_loss_function(recon_batch, data, mu, logvar)
            loss_flow, log_p, logdet = flow_loss_function(epsilon, logdet)
            loss += loss_flow
            train_logdet += logdet.item()
    ...
    print('Average logdet: {:.4f} Average log_p: {:.4f}'.format(
        train_logdet/len(train_iter.dataset), train_log_p/len(train_iter.dataset)))
